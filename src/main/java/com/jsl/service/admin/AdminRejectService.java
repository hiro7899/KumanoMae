...
public class AdminRejectService implements Command {
    ...
    public void doCommand(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String memo = request.getParameter("rejectMemo"); // 반려 사유, 선택값
        ...
        try (Connection conn = DBManager.getConnection()) {
            try {
                ...
                if (!"W".equals(currentStatus)) {
                    throw new AdminActionException("承認待ち状態の通報のみ却下できます。");
                }

                boardDao.updateStatus(conn, boardId, "N");
                adminLogDao.insertLog(conn, adminId, "BOARD", boardId, "REJECT", memo);
                ...
                throw new RuntimeException("却下処理中にエラーが発生しました。", e);
            }
        } catch (SQLException e) {
