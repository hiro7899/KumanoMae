...
public class CommunityBoardDao {
    ...
    public int countActive() {
        String sql = "SELECT COUNT(*) FROM COMMUNITY_BOARD WHERE STATUS = 'Y'";
        try (Connection conn = DBManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql);
    ...
    /** 관리자용 - 상태(Y/N) 무관하게 전체 조회, statusFilter가 null/"all"이면 전부 */
    public List<CommunityBoardDto> selectAllForAdmin(String statusFilter) {
        StringBuilder sql = new StringBuilder("""
            SELECT C_BOARD_ID, MEMBER_ID, CATEGORY, TITLE, GEAR_NAME, VIEW_CNT, LIKE_CNT, STATUS, REG_DATE
              FROM COMMUNITY_BOARD
            """);
        boolean filter = statusFilter != null && !statusFilter.isEmpty() && !"all".equals(statusFilter);
        if (filter) sql.append(" WHERE STATUS = ?");
        sql.append(" ORDER BY REG_DATE DESC");
        ...
        try (Connection conn = DBManager.getConnection();
            ...
            if (filter) pstmt.setString(1, statusFilter);
    ...
    /** 관리자용 - STATUS 무관하게 단건 조회 (일반 selectById()는 STATUS='Y'만 조회함) */
    ...
    /** 관리자 완전삭제용 - 물리 DELETE. FK의 ON DELETE CASCADE로 FILE/COMMENT/LIKE도 함께 삭제됨 */
    public int deletePhysically(Connection conn, Long cBoardId) throws SQLException {
        String sql = "DELETE FROM COMMUNITY_BOARD WHERE C_BOARD_ID = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setLong(1, cBoardId);
            return pstmt.executeUpdate();
        }
