...
public class EmailUtil {
    ...
    static {
        ...
        try (InputStream in = EmailUtil.class.getClassLoader().getResourceAsStream("mail.properties")) {
            if (in == null) {
                throw new ExceptionInInitializerError("mail.properties를 찾을 수 없습니다 (WEB-INF/classes 확인).");
            }
            config.load(in);
        } catch (IOException e) {
            throw new ExceptionInInitializerError("mail.properties 로딩 실패: " + e.getMessage());
        }
    ...
    public static void send(String toEmail, String subject, String htmlBody) {
        ...
        try {
            ...
            message.setFrom(new InternetAddress(SENDER_EMAIL, "KumanoMae"));
